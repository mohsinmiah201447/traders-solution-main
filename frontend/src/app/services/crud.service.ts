...
export class CrudService {
  ...
  save(requestData: any, endPoint: string): Observable<AppResponse<any>> {
    return this.http.post<AppResponse<any>>(`${this.config.apiUrl}/${endPoint}/save`, requestData, {
      headers: {
  ...
  getAll(endPoint: string): Observable<AppResponse<any>> {
    const url = `${this.config.apiUrl}/${endPoint}/list`;
    return this.http.get<AppResponse<any>>(url);
  }
  
  getById(id: number, endPoint: string): Observable<AppResponse<any>> {
    const url = `${this.config.apiUrl}/${endPoint}/find/${id}`;
    return this.http.get<AppResponse<any>>(url);
  }
